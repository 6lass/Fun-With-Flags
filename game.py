...
class GuessTheFlagApp(tk.Tk):
    ...
    def start_game(self):
        ...
        def draw():
            ...
            self.lbl_q_val = tk.Label(l_stats, text="Pregunta 1", fg=TEXT, bg=CARD_BG, font=("Segoe UI", 12, "bold"))
            self.lbl_q_val.pack(anchor="w")
            self.lbl_score_val = tk.Label(l_stats, text="0 pts", fg=BLUE, bg=CARD_BG, font=("Segoe UI", 11, "bold"))
            ...
            self.btn_next = Btn(self, text="Siguiente Bandera ➡️", bg=BLUE, hover_bg="#0369a1", font=("Segoe UI", 12, "bold"), command=self.spawn_question)
    ...
    def spawn_question(self):
        ...
        self.lbl_q_val.configure(text=f"Pregunta {self.total_questions}")
        self.lbl_score_val.configure(text=f"{self.score} pts")
        ...
        self.lbl_flag.configure(image="", text="Cargando...", fg=MUTED)
        ...
        if path and os.path.exists(path):
            try:
                ...
                self.lbl_flag.configure(image="", text=f"[{country_data['nombre']}]", fg=TEXT)
        else:
            self.lbl_flag.configure(image="", text=f"[{country_data['nombre']}]", fg=TEXT)
            
        for i, b in enumerate(self.opt_btns):
            b.configure(text=f"{i+1}.  {FLAGS_DB[self.current_options[i]]['nombre']}")
    ...
    def evaluate_choice(self, idx):
        ...
        self.btn_next.configure(
            text="Resultados 🏁" if self.lives <= 0 else "Siguiente Bandera ➡️",
    ...
    def end_game(self):
        def draw():
            ...
            tk.Label(self, text="Partida Terminada", fg=RED, bg=BG, font=("Segoe UI", 18, "bold")).pack(pady=(15, 5))
            sf = tk.Frame(self, bg=CARD_BG, highlightbackground=BORDER, highlightthickness=1)
            sf.pack(fill="x", padx=20, pady=5, ipady=5)
            tk.Label(sf, text=f"{self.score} pts", fg=BLUE, bg=CARD_BG, font=("Segoe UI", 24, "bold")).pack()
            grid = tk.Frame(sf, bg=CARD_BG)
            grid.pack(fill="x", pady=2)
            grid.columnconfigure((0, 1, 2), weight=1)
            stats = [(self.correct_count, "Aciertos", 0), (f"{acc}%", "Precisión", 1), (f"🔥 {self.max_streak}", "Racha", 2)]
            ...
            if self.score > 0:
                ...
                entry.insert(0, "Jugador")
                def save():
                    self.save_record_local(entry.get()[:15].strip() or "Jugador", self.score, f"{acc}%")
                    messagebox.showinfo("Récord", "¡Guardado!")
                    self.end_game()
                Btn(reg, text="Registrar", bg=BLUE, hover_bg="#0369a1", font=("Segoe UI", 9, "bold"), command=save).pack(side="right")
            lf = tk.Frame(self, bg=CARD_BG, highlightbackground=BORDER, highlightthickness=1)
            lf.pack(fill="both", expand=True, padx=20, pady=5)
            tk.Label(lf, text="Mejores Puntuaciones", fg=TEXT, bg=CARD_BG, font=("Segoe UI", 11, "bold")).pack(pady=4)
            records = self.load_records()
            if not records:
                tk.Label(lf, text="Sin récords aún. ¡Sé el primero!", fg=MUTED, bg=CARD_BG, font=("Segoe UI", 9, "italic")).pack(pady=20)
            else:
                hdr = tk.Frame(lf, bg="#27272a")
                hdr.pack(fill="x", ipady=2)
                tk.Label(hdr, text="Pos", fg=MUTED, bg="#27272a", font=("Segoe UI", 8, "bold"), width=5).pack(side="left")
                tk.Label(hdr, text="Nombre", fg=MUTED, bg="#27272a", font=("Segoe UI", 8, "bold"), width=20, anchor="w").pack(side="left")
                tk.Label(hdr, text="Puntos", fg=MUTED, bg="#27272a", font=("Segoe UI", 8, "bold"), width=12).pack(side="left")
                tk.Label(hdr, text="Precisión", fg=MUTED, bg="#27272a", font=("Segoe UI", 8, "bold"), width=10).pack(side="left")
            ...
            Btn(bf, text="🔄 Jugar de Nuevo", bg=GREEN, hover_bg="#16a34a", font=("Segoe UI", 11, "bold"), command=self.start_game).pack(fill="x", side="left", expand=True, padx=(0, 5), ipady=6)
            Btn(bf, text="❌ Salir", bg="#3f3f46", hover_bg="#52525b", font=("Segoe UI", 11, "bold"), command=self.quit).pack(fill="x", side="left", expand=True, padx=(5, 0), ipady=6)
        self.show_screen(draw)
